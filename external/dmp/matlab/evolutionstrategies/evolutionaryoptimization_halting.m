...
function [theta_opt learning_history] = evolutionaryoptimization_halting(task,task_solver,theta_init,covar_init,n_updates,n_samples,update_parameters,varargin)
% Input:
...
while (i_update<=n_updates)
  ...
  for i_dof=1:n_dofs
    theta_eps(i_dof,:,:) = generate_samples(distributions(i_dof),n_samples,first_is_mean);
  end
    
  %------------------------------------------------------------------
  % Perform rollouts for the samples in theta_eps
  cost_vars = task_solver.performRollouts(theta_eps); 
...
% Test function
  function [theta_opt learning_history] = testevolutionaryoptimization
    ...
    task_solver = TaskSolverFunction();
