...
#include <boost/filesystem.hpp> // Needed only for train(inputs,outputs,save_directory)
...
ostream& FunctionApproximator::serialize(ostream& output) const { 

  output << "{ \"FunctionApproximator" << getName() << "\": {";

  output << "\"meta_parameters\": ";; 
  if (meta_parameters_!=NULL)
    output << *meta_parameters_;
  else
    output << "{}";
    
  output << ", \"model_parameters\" :"; 
  if (model_parameters_!=NULL)
    output << *model_parameters_; 
  else
    output << "{}";
