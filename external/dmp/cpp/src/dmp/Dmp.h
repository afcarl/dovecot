...
public:
  ...
  enum DmpType { IJSPEERT_2002_MOVEMENT, KULVICIUS_2012_JOINING  };
  ...
  /** 
   * Deserialize (i.e. read) a Dmp from an input stream.
   * \param[in] input The input stream (which will be modified due to reading from it)
   * \return A pointer to a new Dmp that was read from the input stream
   */
  static Dmp* deserialize(std::istream& input);
