...
public:
   ...
   *
   *  \remarks Calls pure virtual function serialize(), which must be implemented by
  ...
  friend std::ostream& operator<<(std::ostream& output, const DynamicalSystem& dyn_sys) {
    return dyn_sys.serialize(output);
  }

  /** Writes a DynamicalSystem to an output stream.
...
The evolution of many dynamical systems can also be determined analytically, by explicitely solving the differential equation. For instance, \f$N(t) = x_0e^{-\alpha t}\f$ is the solution
